...
public class CreateMangaRule
{
    ...
    public static void main(String args[]) throws InterruptedException, FileNotFoundException, IOException
    {
        ...
        saveNewSeries("http://www.readmanga.today/mahouka-koukou-no-rettousei-tsuiokuhen", "mahouka-koukou-no-rettousei-tsuiokuhen", "download");
    }
    ...
    public static void saveNewSeries(String url, String name, String downloadTo) throws InterruptedException, IOException
    {
        ...
        System.out.println("#########################################################################################################");
        System.out.println("   CREATING NEW MANGA RULE - " + url);
        System.out.println("#########################################################################################################\n");
        //only run if provided url is readcomiconline or kissmanga
        if (url.toLowerCase().contains("readmanga.today"))
        {
            ...
            try
            {
                ...
                print = new PrintWriter(FileManager.launchPath() + "\\rules\\manga\\" + name + ".xml");
            ...
            } catch (FileNotFoundException ex)
            {
                ...
                Logger.getLogger(CreateMangaRule.class.getName()).log(Level.SEVERE, null, ex);
            } finally
            {
        ...
        } else
        {
            ...
            ColorCmd.println("ENTER VALID URL - ONLY READMANGA.TODAY IS SUPPORTED!", fgRedBgWhite);
            JOptionPane.showMessageDialog(null, "ENTER VALID URL - ONLY READMANGA.TODAY IS SUPPORTED!", "ERROR", JOptionPane.WARNING_MESSAGE);
    ...
    public static List getDetails(String URL) throws InterruptedException, IOException
    {
        ...
        File imageFile = new File(FileManager.launchPath() + "\\graphics\\manga-covers\\" + seriesTitle.replaceAll("[^a-zA-Z0-9. -]", "_") + ".jpg");
        imagedownloader.saveAs(imageFile);
        localImagePath = "\\graphics\\manga-covers\\" + seriesTitle.replaceAll("[^a-zA-Z0-9. -]", "_") + ".jpg";

        //get comic summary
        Element sum = doc.select("li.movie-detail").first();
        ColorCmd.println("Summary: " + sum.text(), fgYellowBgWhite);
        summary = sum.text();
        //get comic status
        Element stat = doc.select("dl.dl-horizontal").select("dd").get(1);
        ...
        Elements links = doc.select("ul.chp_lst").select("a");
        
        for (Element link : links)
        {
            ColorCmd.println(link.select("span").first().text() + ": " + link.select("a").attr("href"), fgYellowBgWhite);
            issues.add("<name>" + link.select("span").first().text() + "</name><link>" + link.select("a").attr("href") + "</link><downloaded>false</downloaded>");
        }
