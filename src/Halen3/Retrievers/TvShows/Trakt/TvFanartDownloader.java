 ...
 * @author brenn
 */
public class TvFanartDownloader
{
    ...
    public static void download(String fanarturl) throws IOException
    {
        ...
        for (Element value : seasons)
        {
            if (value.attr("href").contains("hdtvlogo"))
            {
                ...
                fos.write(response);
                fos.close();
                imagePath =  "\\graphics\\tv-show-logos\\"+ value.attr("href").substring(value.attr("href").lastIndexOf("/")+1, value.attr("href").lastIndexOf("-")).replace("-", " ") + ".png";
                break;
