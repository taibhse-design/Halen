...
import Halen3.IO.SendEmailNotification;
...
public class ThreadedDownload
{
    ...
    static final File animeFolder = new File(Halen3.IO.FileManager.launchPath() + "/rules/anime/");
    static final File[] animeList = animeFolder.listFiles();
    static Thread anime[] = new Thread[animeList.length];
    static final File comicsFolder = new File(Halen3.IO.FileManager.launchPath() + "/rules/comics/");
...
//if string magnet has magnet url then update ep list to indcare its retrieved
                                if (magnet.contains("magnet:?xt="))
                                {
                                    ...
                                    SendEmailNotification.retrievedEpisodes.add(tvList[i].getName().replace(".xml", "") + " " + eps.getItem(j).substring(eps.getItem(j).indexOf("<") + 1, eps.getItem(j).indexOf(">")));
            ...
            anime[i] = new Thread(new Runnable()
            {
                ...
                public void run()
                {
                    ...
                    try
                    {
                        ...
                        List eps = Halen3.IO.FileManager.readFile(animeList[i].getAbsolutePath());
                        for (int j = 1; j < eps.getItemCount(); j++)
                        {
...
//if string magnet has magnet url then update ep list to indcare its retrieved
                                ...
                                {
                                    SendEmailNotification.retrievedAnime.add(animeList[i].getName().replace(".xml", "") + "   " + eps.getItem(j).substring(eps.getItem(j).indexOf("<") + 1, eps.getItem(j).indexOf(">")));
                                    Handler.addLinkTOMAgnetList(magnet);
                                    eps.replaceItem(eps.getItem(j).replace("false", "true"), j);
                                }
                            }
            ...
            comics[i] = new Thread(new Runnable()
            {
                ...
                public void run()
                {
                    ...
                    try
                    {
                        ...
                        List eps = Halen3.IO.FileManager.readFile(comicsList[i].getAbsolutePath());
                        for (int j = 1; j < eps.getItemCount(); j++)
                        {
                            ...
                            if (eps.getItem(j).contains("false"))
                            {
                             ...
                             //   System.out.println("\n");
                                ...
                                if (magnet.contains("magnet:?xt="))
                                {
                                    SendEmailNotification.retrievedComics.add(comicsList[i].getName().replace(".xml", "") + " " + eps.getItem(j).substring(eps.getItem(j).indexOf("<") + 1, eps.getItem(j).indexOf(">")));
                                    Handler.addLinkTOMAgnetList(magnet);
