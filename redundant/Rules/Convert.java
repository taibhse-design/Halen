        ...
        if (oldRules.exists())
        {
            ...
            if (reply == JOptionPane.YES_OPTION)
            {
                ...
                for (int i = 0; i < rules.getItemCount(); i++)
                {
                    ...
                    {
                        PrintWriter out = new PrintWriter(Halen3.IO.FileManager.launchPath() + "/rules/tv show/" + FileManager.returnTag("name", rules.getItem(i)).replaceAll("[^A-Za-z0-9 ]", "") + ".xml");
                    ...
                    {
                        PrintWriter out = new PrintWriter(Halen3.IO.FileManager.launchPath() + "/rules/comics/" + FileManager.returnTag("name", rules.getItem(i)).replaceAll("[^A-Za-z0-9 ]", "") + ".xml");
                        out.println("<search>" + FileManager.returnTag("search", rules.getItem(i)) + "</search>");
                    ...
                    {
                        PrintWriter out = new PrintWriter(Halen3.IO.FileManager.launchPath() + "/rules/anime/" + FileManager.returnTag("name", rules.getItem(i)).replaceAll("[^A-Za-z0-9 ]", "") + ".xml");
                        out.println("<search>" + FileManager.returnTag("search", rules.getItem(i)) + "</search>");
